...
async function get_date_range(efj) {
    return new Promise((resolve, reject) => {
        ...
        if(dates.length == 0) {
            ...
            ID("dr_from").value = dates[0].toISOString().slice(0, 10);
            ID("dr_to").value = to_date.toISOString().slice(0, 10);
...
async function post(efj, action) {
    let daterange = await get_date_range(efj);
    let response;
    try {
        ...
        if(response.ok) {
            ...
            return [`HTTP error: ${response.status}`, "failed"];
        }
    } catch (error) {
        ID("working").classList.add("hidden");
        return ["Network error", "failed"];
    }
}
